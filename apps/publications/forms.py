...
class StoryCreationForm(ModelForm):
    ...
    class Meta:
        ...
        widgets = {"status": CustomPermissionSelect}
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields[
            "tag"
        ].help_text = """Los tags ayudan a los lectores a encontrar las Storylinks de su interés.
        ...
        self.fields[
            "img_content_link"
        ].help_text = "Los formatos permitidos son jpg, jpeg y png, con un tamaño máximo de 5MB."
        self.fields["color"] = forms.ChoiceField(choices=CHOICES, widget=CustomColorSelect())
        self.fields["color"].initial = "#4a4a4a"
        self.fields[
            "color"
        ].help_text = "Puedes seleccionar un color para personalizar la publicación."
        self.fields["status"].label = "Elige qué acción pueden realizar otros usuarios"
        self.fields[
            "status"
        ].help_text = """Elige cómo es la visibilidad y qué acciones pueden realizar otros 
...
class StoryContinuationCreationForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields[
            "quest_answ"
        ].help_text = """La Pregunta Decisiva ayuda al lector a identificar cómo continúa la trama en esta Storylink.
...
class StoryEditForm(ModelForm):
    ...
    class Meta:
        ...
        fields = ("title", "text_content", "img_content_link", "color", "status")
        error_css_class = "error"
        widgets = {"img_content_link": CustomImageField, "status": CustomPermissionSelect}
    ...
    def __init__(self, *args, **kwargs):
        ...
        if fpub:
            ...
            if fpub.status == Permission.WR:
                ...
                self.fields[
                    "status"
                ].help_text = "<span class='material-icons'>error_outline</span> No se pueden cambiar los permisos de esta publicación."
            elif fpub.status == Permission.R:
                self.fields[
                    "status"
                ].help_text = """Otros usuarios solo pueden leer esta publicación. 
                ...
                self.fields[
                    "status"
                ].help_text = """Solo tú puedes ver esta publicación. 
...
def validate_image(image):
    ...
    if filesize > megabyte_limit * 1024 * 1024:
        raise ValidationError(
            "El tamaño máximo permitido para las imágenes es %sMB" % str(megabyte_limit)
        )
