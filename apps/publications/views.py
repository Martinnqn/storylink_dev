...
class ListUserStories(LoginRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        ...
        qs = StoryPublication.objects.get_stories_by_user(
            user, self.request.user).order_by('date_time__month', '-date_time__day')
...
class StoryContinuations(LoginRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        qs = self.model.objects.story_continuations(
...
class ChapterContinuations(LoginRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        qs = self.model.objects.chapter_continuations(
            self.kwargs["pk"], self.request.user)
        return qs
    ...
    def get(self, *args, **kwargs):
        ...
        if (self.request.is_ajax()):
            ...
            if (not publication.status == Permission.NR or own_user == self.request.user.profile.get()):
                data['content_pub'] = model_to_dict(
                    publication, exclude=['id', 'tag', 'own_user', 'img_content_link', 'like'])
                data['content_pub'].update(
                    {'id': 'story_'+str(publication.id)})
                data['content_pub'].update(
                    {'own_username': own_user.user.username})
                data['content_pub'].update(
                    {'user_name': own_user.user.first_name})
                data['content_pub'].update(
                    {'user_lastname': own_user.user.last_name})
                data['content_pub'].update({'own_user': own_user.id})
                data['content_pub'].update({'own_first_story': own_user.id})
                data['content_pub'].update({'url_delete': reverse_lazy('user:pub:delete_story', kwargs={
                                           'username': own_user.user.username, 'pk': publication.id})})
                data['content_pub'].update({'url_edit': reverse_lazy('user:pub:edit_story', kwargs={
                                           'username': own_user.user.username, 'pk': publication.id})})
                data['content_pub'].update({'url_subscribe': reverse_lazy('user:pub:subs_story', kwargs={
                                           'username': own_user.user.username, 'pk': publication.id})})
                data['content_pub'].update({'url_unsubscribe': reverse_lazy('user:pub:unsubs_story', kwargs={
                                           'username': own_user.user.username, 'pk': publication.id})})
                data['content_pub'].update({'url_like': reverse_lazy('user:pub:like_story', kwargs={
                                           'username': own_user.user.username, 'pk': publication.id})})
                data['content_pub'].update({'url_unlike': reverse_lazy('user:pub:unlike_story', kwargs={
                                           'username': own_user.user.username, 'pk': publication.id})})
                data['content_pub'].update({'url_continuate': reverse_lazy('user:pub:create_story_cont', kwargs={
                                           'username': own_user.user.username, 'pk': publication.id})})
                data['content_pub'].update({'url_continuations': reverse_lazy(
                    'user:pub:conts_story', kwargs={'username': own_user.user.username, 'pk': publication.id})})
                data['content_pub'].update({'url_autor': reverse_lazy(
                    'user:user_profile', kwargs={'username': own_user.user.username})})
                ...
                profile = own_user
                data['content_pub'].update(
                    {'own_user_image': profile.link_img_perfil.url})
                ...
                if (publication.active):
                    ...
                    data['content_pub'].update(
                        {'img_content_link': publication.img_content_link.url})
                    ...
                    data['content_pub'].update(
                        {'text_content': "No se puede visualizar el contenido de esta Storylink."})
                    data['content_pub'].update(
                        {'img_content_link': default_img})
    ...
    def get(self, *args, **kwargs):
        ...
        if (self.request.is_ajax()):
            ...
            if (not mainStory.status == Permission.NR or own_user == self.request.user.profile.get()):
                data['content_pub'] = model_to_dict(
                    publication, exclude=['id', 'tag', 'own_user', 'like'])
                data['content_pub'].update(
                    {'id': 'chapter_'+str(publication.id)})
                data['content_pub'].update(
                    {'own_username': own_user.user.username})
                data['content_pub'].update(
                    {'user_name': own_user.user.first_name})
                ...
                data['content_pub'].update(
                    {'own_first_story': mainStory.own_user.id})
                data['content_pub'].update(
                    {'own_name_first_story': mainStory.own_user.user.username})
                data['content_pub'].update(
                    {'question': publication.quest_answ})
                data['content_pub'].update({'url_delete': reverse_lazy('user:pub:delete_chapt', kwargs={
                                           'username': own_user.user.username, 'pk': publication.id})})
                data['content_pub'].update({'url_edit': reverse_lazy('user:pub:edit_chapter', kwargs={
                                           'username': own_user.user.username, 'pk': publication.id})})
                data['content_pub'].update({'url_subscribe': reverse_lazy('user:pub:subs_story', kwargs={
                                           'username': own_user.user.username, 'pk': mainStory.id})})
                data['content_pub'].update({'url_unsubscribe': reverse_lazy(
                    'user:pub:unsubs_story', kwargs={'username': own_user.user.username, 'pk': mainStory.id})})
                data['content_pub'].update({'url_like': reverse_lazy('user:pub:like_chapter', kwargs={
                                           'username': own_user.user.username, 'pk': publication.id})})
                data['content_pub'].update({'url_unlike': reverse_lazy('user:pub:unlike_chapter', kwargs={
                                           'username': own_user.user.username, 'pk': publication.id})})
                data['content_pub'].update({'url_continuate': reverse_lazy('user:pub:create_story_cont', kwargs={
                                           'username': own_user.user.username, 'pk': mainStory.id, 'pkchapter': publication.id})})
                data['content_pub'].update({'url_continuations': reverse_lazy(
                    'user:pub:conts_chap', kwargs={'username': own_user.user.username, 'pk': publication.id})})
                data['content_pub'].update({'url_first_story': reverse_lazy(
                    'user:pub:story_content', kwargs={'username': own_user.user.username, 'pk': mainStory.id})})
                data['content_pub'].update({'url_autor': reverse_lazy(
                    'user:user_profile', kwargs={'username': own_user.user.username})})
                data['content_pub'].update({'url_autor_init': reverse_lazy(
                    'user:user_profile', kwargs={'username': mainStory.own_user.user.username})})
                ...
                is_liked = fromUser.profile.get().likeToChapter.filter(
                    to_chapter=publication).exists()
                data['content_pub'].update({'is_liked': is_liked})
                ...
                if (prev):
                    data['content_pub'].update(
                        {'previous_pub_id': 'chapter_'+str(prev.id)})
                    data['content_pub'].update({'url_prev_chapter': reverse_lazy('user:pub:chapter_content', kwargs={
                                               'username': own_user.user.username, 'pk': publication.prevChapter.id})})
                else:
                    data['content_pub'].update(
                        {'previous_pub_id': 'story_'+str(mainStory.id)})
                ...
                if (publication.active):
                    ...
                    data['content_pub'].update({'tags': tags})
                    data['content_pub'].update(
                        {'img_content_link': mainStory.img_content_link.url})
                else:
                    data['content_pub'].update({'tags': []})
                    data['content_pub'].update(
                        {'text_content': "No se puede visualizar el contenido de esta Storylink."})
...
class CreateStoryContinuation(LoginRequiredMixin, generic.DetailView, generic.CreateView):
    ...
    def form_valid(self, form):
        storyMain = get_object_or_404(
            StoryPublication, id=self.kwargs.get('pk'))
            ...
            try:
                with transaction.atomic():
                    ...
                    if (self.kwargs.get('pkchapter')):
                        prevChap = get_object_or_404(
                            StoryChapter, id=self.kwargs.get('pkchapter'))
...
class StoryContinuationsTitle(LoginRequiredMixin, View):
    ...
    def get(self, *args, **kwargs):
        if (self.request.is_ajax()):
            conts = self.model.objects.story_continuations(
                self.kwargs["pk"], self.request.user)
            data = {'childs': []}
            for c in conts:
...
class ChapterContinuationsTitle(LoginRequiredMixin, generic.DetailView):
    ...
    def get(self, *args, **kwargs):
        if (self.request.is_ajax()):
            conts = self.model.objects.chapter_continuations(
                self.kwargs["pk"], self.request.user)
            data = {'childs': []}
            for c in conts:
                data['childs'] += [{'pubId': c.id, 'title': c.quest_answ}]
