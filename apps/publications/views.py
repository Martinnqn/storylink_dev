...
from apps.publications.forms import (
    StoryCreationForm,
    StoryContinuationCreationForm,
    StoryEditForm,
    StoryChapterEditForm,
)
...
class ListUserStories(LoginRequiredMixin, generic.ListView):
    """List Stories created by the URL username parameter"""
    ...
    def get_queryset(self):
        ...
        qs = StoryPublication.objects.get_stories_by_user(user, self.request.user).order_by(
...
class ListContentStory(LoginRequiredMixin, generic.DetailView):
    """List story content. If is by ajax, returns a json, else return
    ...
    def get(self, *args, **kwargs):
        ...
        if self.request.is_ajax():
            data = dict()
            if (
                not publication.status == Permission.NR
                or own_user == self.request.user.profile.get()
            ):
                ...
                data["content_pub"].update(
                    {
                        "url_delete": reverse_lazy(
                            "user:pub:delete_story",
                            kwargs={"username": own_user.user.username, "pk": publication.id},
                        )
                    }
                )
                data["content_pub"].update(
                    {
                        "url_edit": reverse_lazy(
                            "user:pub:edit_story",
                            kwargs={"username": own_user.user.username, "pk": publication.id},
                        )
                    }
                ...
                data["content_pub"].update(
                    {
                        "url_unsubscribe": reverse_lazy(
                            "user:pub:unsubs_story",
                            kwargs={"username": own_user.user.username, "pk": publication.id},
                        )
                    }
                )
                ...
                data["content_pub"].update(
                    {
                        "url_unlike": reverse_lazy(
                            "user:pub:unlike_story",
                            kwargs={"username": own_user.user.username, "pk": publication.id},
                        )
                    }
                )
                data["content_pub"].update(
                    {
                        "url_continuate": reverse_lazy(
                            "user:pub:create_story_cont",
                            kwargs={"username": own_user.user.username, "pk": publication.id},
                        )
                    }
                )
                data["content_pub"].update(
                    {
                        "url_continuations": reverse_lazy(
                            "user:pub:conts_story",
                            kwargs={"username": own_user.user.username, "pk": publication.id},
                        )
                    }
                )
    ...
    def get(self, *args, **kwargs):
        ...
        if self.request.is_ajax():
            ...
            if not mainStory.status == Permission.NR or own_user == self.request.user.profile.get():
                ...
                data["content_pub"].update(
                    {
                        "url_subscribe": reverse_lazy(
                            "user:pub:subs_story",
                            kwargs={"username": own_user.user.username, "pk": mainStory.id},
                        )
                    }
                )
                data["content_pub"].update(
                ...
                data["content_pub"].update(
                    {
                        "url_continuate": reverse_lazy(
                            "user:pub:create_story_cont",
                            kwargs={
                                "username": own_user.user.username,
                                "pk": mainStory.id,
                                "pkchapter": publication.id,
                            },
                        )
                    }
                )
                ...
                data["content_pub"].update(
                    {
                        "url_first_story": reverse_lazy(
                            "user:pub:story_content",
                            kwargs={"username": own_user.user.username, "pk": mainStory.id},
                        )
                    }
                )
                ...
                data["content_pub"].update(
                    {
                        "url_autor_init": reverse_lazy(
                            "user:user_profile",
                            kwargs={"username": mainStory.own_user.user.username},
                        )
                    }
                )
                ...
                is_liked = (
                    fromUser.profile.get().likeToChapter.filter(to_chapter=publication).exists()
                )
                data["content_pub"].update({"is_liked": is_liked})
                ...
                if prev:
                    ...
                    data["content_pub"].update(
                        {
                            "url_prev_chapter": reverse_lazy(
                                "user:pub:chapter_content",
                                kwargs={
                                    "username": own_user.user.username,
                                    "pk": publication.prevChapter.id,
                                },
                            )
                        }
                    )
...
class DeleteStory(LoginRequiredMixin, generic.edit.DeleteView):
    """story soft delete. They are not delete, they just become inactive"""
...
class DeleteChapter(LoginRequiredMixin, generic.edit.DeleteView):
    """Delete chapter. They are not delete, they just become inactive"""
...
class EditStory(LoginRequiredMixin, generic.edit.UpdateView):
    ...
    def get(self, *args, **kwargs):
        ...
        if story.own_user != self.request.user.profile.get():
            return redirect(
                reverse_lazy("user:user_profile", kwargs={"username": self.request.user.username})
            )
        else:
            return super().get(*args, **kwargs)
...
class CreateStoryContinuation(LoginRequiredMixin, generic.DetailView, generic.CreateView):
    """Create chapter. This class require detail and create view.
    Create view for get a form corresponding to the
    StoryContinuationCreationForm, junto con su form_isvalid().
    Detailview for get the StoryPublication referred to in the chapter. The
    StoryPublication is obtained by Django automatically through the pk of the
    url."""
    ...
    def form_valid(self, form):
        ...
        if storyMain.status == Permission.WR or (
            self.request.user.profile.get() == storyMain.own_user
        ):
            try:
                with transaction.atomic():
                    story = form.save(commit=False)
...
class SubscribeStory(LoginRequiredMixin, View):
    """Add subscription from user to story."""
...
class UnsubscribeStory(LoginRequiredMixin, View):
    """Delete subscription from user to story."""
...
class UnlikeStory(LoginRequiredMixin, View):
    """Delete like from user to story"""
...
class UnlikeChapter(LoginRequiredMixin, View):
    """Delete like from user to chapter"""
...
class ListStories(LoginRequiredMixin, FormMixin, generic.ListView):
    ...
    def get_queryset(self):
        qs = StoryPublication.objects.publications_hall(
            self.request.GET.get("title", None), self.request.GET.get("tag", None)
        )
        return qs
