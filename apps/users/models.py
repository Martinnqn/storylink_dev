...
# From apps.publications.models import StoryPublication #import circular!
# Solucionado con app_level.ModelName (publications.StoryPublication)

# funcion para validar si el usuario es unico (equalsIgnoreCase) y no contiene
# punto
...
def unique_user(value):
    if (not check_characters(value)):
        raise ValidationError(
            _('''No se permiten utilizar caracteres especiales como ", ?,¡, &, %%,
             ., comas, espacios, etc en los nombres de usuario.'''),
            params={'value': value},
        )
...
class CustomUser(AbstractUser):
    ...
    username = models.CharField(
        'Nombre de usuario', unique=True, max_length=35,
...
class UserProfile(models.Model):
    ...
    link_img_perfil = models.ImageField(
        upload_to=get_upload_path, default='gallery/no-img-profile.png',
        max_length=350)
    description = models.CharField(max_length=150, blank=True)
    # a que usuario esta suscripto. Se usa un modelo intermedio para
    #  especificar los atributos de la tabla intermedia.
    user_subscription = models.ManyToManyField(
        'self', through='UserSubscriptionModelAux', symmetrical=False)
    # a que publicacion esta suscripto. Se usa un modelo intermedio para
    #  especificar los atributos de la tabla intermedia.
    pub_subscription = models.ManyToManyField(
        'publications.StoryPublication', through='PubSubscriptionModelAux',
        symmetrical=False)
    # si el usuario tiene la cuenta reportada
    is_reported = models.BooleanField(default=False)

# modelo intermedio para las suscripciones de un usuario a otro usuario. 
# from_user esta suscripto a to_user
